...
if (exist('patch_mode','var') && patch_mode)
    ...
    if (exist('interactive') && interactive)
        ...
        while 1
            ...
            % permute
            patchA = permute(patchA,[2,1,3]);
            patchB = permute(patchB,[2,1,3]);
            %             for dim = 1 : dA
...
k_addition = ''; if K_of_KNN > 1, k_addition = ' (for match 1 out of k)'; end
h_info = figure;
dispX = CSH_Mapping(1:end - br_boundary_to_ignore,1:end - br_boundary_to_ignore,1);
subplot 221; imagesc(dispX); colorbar; title(['X disparities' k_addition]); axis image
dispY = CSH_Mapping(1:end - br_boundary_to_ignore,1:end - br_boundary_to_ignore,2);
subplot 222; imagesc(dispY); colorbar; title(['Y disparities' k_addition]); axis image
subplot 223
hist(errImg(:),100); title('distribution of avg. erros');
subplot 224; 
targetBin = zeros(hB,wB);
for kInd = 1 : K_of_KNN
    tXs = double(CSH_Mapping(1:end - br_boundary_to_ignore,1:end - br_boundary_to_ignore,1,kInd));
    tYs = double(CSH_Mapping(1:end - br_boundary_to_ignore,1:end - br_boundary_to_ignore,2,kInd));
    targetBin(sub2ind([hB,wB],tYs,tXs)) =1;
end
imagesc(targetBin);
title('imageB hit target locations (red)');
set(h_info,'name','moreInfo');
