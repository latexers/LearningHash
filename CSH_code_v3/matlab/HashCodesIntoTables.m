...
if (useC) % (these are MEX functions)
    MexFillTable(hTables_A,indices_A_flat,uint16(hashBits), uint16(hA),uint16(wA),uint16(indices_A_offset),uint16(numHashs)); % designed for numHashs=2;
    MexFillTable(hTables_B,indices_B_flat,uint16(hashBits), uint16(hB),uint16(wB),uint16(indices_B_offset),uint16(numHashs)); % designed for numHashs=2;
else
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
