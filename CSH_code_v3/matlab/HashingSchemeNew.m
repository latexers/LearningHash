...
if (k>1) && (calcBnn)
    error('This combination of ''(k>1) && (calcBnn)'', currently doesn''t work due to a bug. compute for each direction separately...');
end
...
descriptor_mode = parameters.descriptor_params.descriptor_mode;
maxKernels = parameters.maxKernels;
useTics = parameters.useTicsInside;
TLboundary = parameters.TLboundary;
BRboundary = parameters.BRboundary;
br_boundary_to_ignore = parameters.br_boundary_to_ignore;
numTables = parameters.numHashTables;
numHashs = parameters.numHashs;
...
if isfield(parameters,'KNN_enrichment_mode')
...
if (insideInfo)
    d_mapping = d_mapping + 1; % Add the error dimension in case of debug
end
...
% GRTIME = tic;
% Output PCA projections is in class single
...
if (k > 1)
    ...
    % candidates
    if ~exist('KNN_enrichment_mode','var')
...
% in the memory for the ranknig stage: matrix of [number_of_filter *  (Width* Height)]
        ...
        if (k > 1)
            if strcmp(classType,'int16')
                if (rotation_invariant)
                    if (dist > 0)
                        alg = @GCKCodebookPropagation16s_KNN_B_Rot_Inv_with_dist;
                    else
                        alg = @GCKCodebookPropagation16s_KNN_B_Rot_Inv;
                    end
                else
            ...
            if (dist > 0)
                alg(bestErrorsNewA, nBestMapping32uA, CF_Vectors_A, CF_Vectors_B, indices_A, indices_B, hTables_A, hTables_B, ...
                    nHashsNoe16u, nSequencyOrder16u, nSequencyLevels16u, bDownwardFlag, uint32(k),int16(dist));
            else
            alg(bestErrorsNewA, nBestMapping32uA, CF_Vectors_A, CF_Vectors_B, indices_A, indices_B, hTables_A, hTables_B, ...
